...
import { 
  ...
  MoreVertical, Share2, Download, Trash2, Shield, RotateCcw
} from 'lucide-react';
...
export default function FileGrid({ 
  ...
  onShare?: (id: string, status: boolean) => void,
  isTrash?: boolean
}) {
  if (loading) return (
       ...
       {files.map((file, i) => (
         ...
         >
           ...
           <div className="mt-6 flex items-center justify-between relative z-10">
              ...
              <div className="flex gap-2">
                 {file.is_shared && <Share2 size={14} className="text-blue-400" />}
           ...
           <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex gap-2 opacity-0 group-hover:opacity-100 transition-all translate-y-4 group-hover:translate-y-0 scale-90 group-hover:scale-100 z-20">
              ...
              <ActionButton 
                icon={<Share2 size={16} />} 
                label={file.is_shared ? "UNSHARE" : "SHARE"} 
                onClick={(e: any) => {
                  e.stopPropagation();
                  if (onShare) onShare(file.id, !file.is_shared);
                }} 
              />
           </div>
           )}
